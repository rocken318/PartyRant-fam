# Code Review Prompt — PartyRant-fam
...
- **特有の機能**:
  - `/solo` — ソロプレイ（ログイン不要、学年・教科・問題数を選んでランダムクイズ）
  - `/presets` — プリセット一覧（学年グループ・教科フィルター、ランダム雑学カードに学年範囲選択あり）
  - `/api/quiz/random` — 学年範囲（gradeMin/gradeMax）と教科でフィルターしてQuestion[]を返す
  - `/api/trivia/random` — 同上フィルター＋sceneでランダムライブゲームを作成
- **リアルタイム通信**: サーバー側でSupabase REST Broadcastを送信、クライアント側はSSE（`/api/stream/[gameId]`）でSubscribe
- **問題データ**: `games`テーブルの`questions` JSONB列に`grade`（1-12）・`subject`フィールドを持つプリセットゲーム（`is_preset=true`）818件
...
- `/api/quiz/random`でgradeMin > gradeMaxになりうるケースのバリデーション
...
- プリセット818件を全件取得している箇所でのパフォーマンス（フィルターをDBレベルに移せないか）
...
6. `src/app/solo/page.tsx` と `/api/quiz/random` の学年・教科フィルターのエッジケースを確認する
